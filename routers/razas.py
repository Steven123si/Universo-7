...
def crear_raza(raza: RazaCrear, usuario: dict = Depends(obtener_usuario_actual)):
    conexion = obtener_conexion()
    cursor = conexion.cursor()
    
    # Validar existencia del planeta de origen (Llave foránea)
...
def actualizar_raza(id: int, raza: RazaCrear, usuario: dict = Depends(obtener_usuario_actual)):
    conexion = obtener_conexion()
    cursor = conexion.cursor()
...
def eliminar_raza(id: int, usuario: dict = Depends(obtener_usuario_actual)):
    conexion = obtener_conexion()
    cursor = conexion.cursor()
...
# Endpoint con JOIN: Obtiene un planeta con todas sus razas originarias
@router.get("/planeta/{planeta_id}", response_model=PlanetaConRazasRespuesta)
def obtener_planeta_con_razas(planeta_id: int):
    conexion = obtener_conexion()
    cursor = conexion.cursor()
